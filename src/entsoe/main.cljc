(ns entsoe.main
  "Kotodama WASM entrypoint for the Entsoe clean-room actor (L4) — Clojure port.
  ...
  facts (`entsoe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "entsoe")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"entsoe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "entsoe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :entsoe-compat/ready)
